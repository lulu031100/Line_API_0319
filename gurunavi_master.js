'use strict';
//ぐるなびの小分類カテゴリを表示する
//ReferenceError: options is not defined でnodeできず
...
//Line Botに入力されたメッセージがテキストであることを確認する
  if ((event.message.type !== 'text') || (event.message.type !== 'text')) {
    return Promise.resolve(null);
  }
...
const gurunaviSearch = async (userId, latitude, longitude) => {
  ...
  const url = 'https://api.gnavi.co.jp/master/CategorySmallSearchAPI/v3/';
  const format = 'json';
  //const range = 1;              // 緯度/経度からの検索範囲(半径)。1は、300m
  const hit_per_page = 100;       // 検索結果の最大取得件数
    ...
    if(!err && response.statusCode == 200) {
      ...
      if (json.rest) {
        ...
        Object.keys(list).some(function(key) {
          ...
          const item = list[category_s];

          let name = item.category_s_code;
          if (name) {
            message += 'No.' + number + NEW_LINE;
            message += '◆小カテゴリ：' + NEW_LINE;
            message += name + NEW_LINE;

            let opentime = item.category_s_name;
            if (opentime) {
              message += '◆名前：' + NEW_LINE;
